function [lgraph, info] = CConvOutput3D(info)
...
finalConvR = convolution3dLayer(1, 1,...
    'BiasL2Factor',0,...
    'WeightsInitializer', 'glorot',...
    'BiasInitializer', 'zeros',...
    'Name','Final-ConvolutionLayerR');

finalConvR.Weights = f  * randn(1,1,1,YR_channels,1);
finalConvR.Bias = zeros(1,1,1,1);

lgraph = addLayers(lgraph, finalConvR);

finalConvI = convolution3dLayer(1, 1,...
    'BiasL2Factor',0,...
    'WeightsInitializer', 'glorot',...
    'BiasInitializer', 'zeros',...
    'Name','Final-ConvolutionLayerI');

finalConvI.Weights = f  * randn(1,1,1,YI_channels,1);
finalConvI.Bias = zeros(1,1,1,1);

lgraph = addLayers(lgraph, finalConvI);

%% 
lgraph = connectLayers(lgraph,reluR_name,'Final-ConvolutionLayerR');
lgraph = connectLayers(lgraph,reluI_name,'Final-ConvolutionLayerI');
end
