function [lgraph, info] = CConvInput3D(info,inputTileSize)
...
input_R = splittingLayer('ImageInputLayerR','1st');
input_I = splittingLayer('ImageInputLayerI','2nd');

lgraph = addLayers(lgraph, input_R);
lgraph = addLayers(lgraph, input_I);
%% parameters definition. 
...
%% Connect Layers
lgraph = connectLayers(lgraph,'ImageInputLayer','ImageInputLayerR');
lgraph = connectLayers(lgraph,'ImageInputLayer','ImageInputLayerI');

lgraph = connectLayers(lgraph,'ImageInputLayerR',conv_RR);
lgraph = connectLayers(lgraph,'ImageInputLayerI',conv_II);
lgraph = connectLayers(lgraph,'ImageInputLayerR',conv_RI);
lgraph = connectLayers(lgraph,'ImageInputLayerI',conv_IR);
