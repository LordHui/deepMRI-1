...
%          imageDatastore, pixelLabelImageDatastore,
%          matlab.io.datastore.TransformedDatastore

% Copyright 2018-2019 The MathWorks, Inc.

classdef YangrandomPatchExtractionDatastore < ...
        matlab.io.Datastore &...
    ...
    methods
        ...
        function this = randomPatchExtractionDatastore(ds1,ds2,patchSize,varargin)
            ...
            elseif isa(ds1,'matlab.io.datastore.ImageDatastore') && isa(ds2,'matlab.io.datastore.ImageDatastore')
                ...
                if ~(isInputOrUnderlyingImageOrPixelLabelDatastore(this.dsFirstInternal) && ...
                        isInputOrUnderlyingImageOrPixelLabelDatastore(this.dsSecondInternal))
                    error(message('images:randomPatchExtractionDatastore:invalidImageBasedInputDatastores'));
                end
                
                numPartitionsFirst = numpartitions(this.dsFirstInternal);
                numPartitionsSecond = numpartitions(this.dsSecondInternal);
            ...
            if isInputAssociatedImageDatastore(this.DatastoreInternal) || ...
                    isInputPixelLabelImageDatastore(this.DatastoreInternal)
        ...
        function numObservations = get.NumObservations(this)
            ...
            % combined datastores
            if isInputCombinedDatastore(this.DatastoreInternal)
                numObservations = [];
            else
    ...
    methods
        ...
        function info = populateOutputInfoStructForThisPatch(this, info, patchIdx, imgIndex, patchLocation)
            ...
            if isInputPixelLabelImageDatastore(this.DatastoreInternal) || ...
                    isInputAssociatedImageDatastore(this.DatastoreInternal)
        ...
        function dataVariableNames = getVariableNamesOfData(this)
            ...
            elseif isInputAssociatedImageDatastore(this.DatastoreInternal)
        ...
        function newds = shuffle(this)
            ...
            if ~isshuffleable(this)
                warning(message('images:randomPatchExtractionDatastore:unsupportedMethod','shuffle'));
                return;
            end
            
            % Assign the ordered patch indices from the orginal datastore
    ...
    methods (Hidden)
        function frac = progress(this)
            frac = this.CurrentPatchIndex / this.NumObservations;
        end
        
        function TF = isshuffleable(this)
            % Any input datastore pair that resuts in a CombinedDatastore
            % is not shuffleable
            TF = ~isInputCombinedDatastore(this.DatastoreInternal);
        end
        
        function TF = ispartitionable(this)
            
            TF = ( matlab.io.datastore.internal.shim.isPartitionable(this.dsFirstInternal) && ...
                   matlab.io.datastore.internal.shim.isPartitionable(this.dsSecondInternal) ) ...
                || ...
                ( isInputOrUnderlyingImageOrPixelLabelDatastore(this.dsFirstInternal) && ...
                  isInputOrUnderlyingImageOrPixelLabelDatastore(this.dsSecondInternal) );
        end
    end
...
%------------------------------------------------------------------
function TF = isInputPixelLabelImageDatastore(ds)
TF = isa(ds,'images.internal.datastore.PixelLabelImageDatastore');
end

%------------------------------------------------------------------
function TF = isInputAssociatedImageDatastore(ds)
TF = isa(ds,'images.internal.datastore.associatedImageDatastore');
end

%------------------------------------------------------------------
function TF = isInputCombinedDatastore(ds)
TF = isa(ds,'matlab.io.datastore.CombinedDatastore');
end

%------------------------------------------------------------------
function TF = isInputOrUnderlyingImageOrPixelLabelDatastore(ds)
% Returns true if input, ds is an imageDatastore or pixelLabelDatastore or
% TransformedDatastore with underlying imageDatastore or
% pixelLabelDatastore

if isa(ds,'matlab.io.datastore.TransformedDatastore')
    TF = isInputOrUnderlyingImageOrPixelLabelDatastore(ds.UnderlyingDatastore);
else
    TF = isInputImageOrPixelLabelDatastore(ds);
end

end

%------------------------------------------------------------------
function TF = isInputImageOrPixelLabelDatastore(ds)
% Returns true if input, ds is an imageDatastore or pixelLabelDatastore

TF = isa(ds,'matlab.io.datastore.PixelLabelDatastore') || ...
